...
Это экспериментальный backend компилятора для явного исследования и проверки инвариантов code generation: живости значений, `phi`-узлов, spills, назначения регистров и корректности результата аллокатора. Проект намеренно компактный: он не имитирует production backend целиком, а изолирует сложные backend-механизмы так, чтобы их можно было проверять отдельными oracle и сравнивать между собой.

Архитектурные решения, инварианты и failure modes вынесены в [`DESIGN.md`](DESIGN.md).
